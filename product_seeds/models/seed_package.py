...
class SeedPackage(models.Model):
    ...
    def create(self, vals):
        ...
        if len(package_attribute) > 1:
            raise ValidationError(_('Several product attributes are set as packages'))  # noqa

        vals['attribute_id'] = package_attribute.pop()

        package = super(SeedPackage,
                        self.with_context(seed_package=True)).create(vals)
        return package
    ...
    def _check_attribute_value(self):
        for package in self:
            if not package.attribute_id.is_package:
                raise ValidationError(_('Attribute must be "package" (is_package == True)'))  # noqa
